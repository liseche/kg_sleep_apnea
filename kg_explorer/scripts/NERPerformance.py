...
def get_flattened_y(gold_df, df_list):
    ...
    for df in df_list:
        ...
        y_pred_flat = [("O" if label == "" else label)
                       for label in y_pred_flat]

        assert len(y_true_flat) == len(
            y_pred_flat), "y_t and y_pred do not have the same length"
...
def metrics_df(y_true_flat: list, y_pred_flat: list, labels: list):
    precision_micro, recall_micro, f1_micro, _ = precision_recall_fscore_support(
        y_true_flat, y_pred_flat, average='micro', labels=labels, zero_division=0)
    precision_macro, recall_macro, f1_macro, _ = precision_recall_fscore_support(
        y_true_flat, y_pred_flat, average='macro', labels=labels, zero_division=0)
    precision_weighted, recall_weighted, f1_weighted, _ = precision_recall_fscore_support(
        y_true_flat, y_pred_flat, average='weighted', labels=labels, zero_division=0)
...
def get_io_bio_disease_filtered_gold(gold_df):
    ...
    assert len(bio_labels) == len(
        entity_labels), "target and pred not same length"
...
def convert_0_to_o(y_pred_flat):
    ...
    assert len(y_pred1) == len(
        y_pred_flat), "len of converted predictions are not the same."
    return y_pred1
...
def compare2gold(gold_df, df_list, files_list):
    ...
    filtered_gold_io, filtered_gold_bio = get_io_bio_disease_filtered_gold(
        gold_df)
    ...
    for df_index in range(len(df_list)):
        ...
        if set(labels) == set(annotation_labels):
            y_random = [random.choice(["O"] + labels)
            ...
            y_temp = ["LABEL_0" if (
                label == "O" or label == "LABEL_0") else "LABEL_1" for label in y_true_flat]
            y_t1 = []
            y_p1 = []

            assert len(y_temp) == len(
                y_pred_flat), "Length of target and pred is not the same"
            ...
            cm = confusion_matrix(
                y_true=y_t, y_pred=y_pred_flat, labels=all_labels)
            ConfusionMatrixDisplay(confusion_matrix=cm,
                                   display_labels=all_labels).plot()
            filename = "cm" + str(fig_counter)
            fig_counter = fig_counter + 1
            plt.savefig(filename)
            print("\nRandom:")
            ...
            cm = confusion_matrix(y_true=filtered_gold_bio,
                                  y_pred=y_pred1, labels=all_labels)
            ConfusionMatrixDisplay(confusion_matrix=cm,
                                   display_labels=all_labels).plot()
            filename = "cm" + str(fig_counter)
            fig_counter = fig_counter + 1
            ...
            for label in labels:
                if "DISEASE" in label:
                    ...
                    y_random = [random.choice(all_labels)
                                for i in range(len(y_pred2))]
                    print(metrics_df(filtered_gold_io, y_pred2, [
                          la for la in all_labels if la != "O"]))
                    cm = confusion_matrix(
                        y_true=filtered_gold_io, y_pred=y_pred2, labels=all_labels)
                    ConfusionMatrixDisplay(
                        confusion_matrix=cm, display_labels=all_labels).plot()
                    filename = "cm" + str(fig_counter)
                    fig_counter = fig_counter + 1
                    plt.savefig(filename)
                    print("\nRandom:")
                    print(metrics_df(filtered_gold_io, y_random, [
                          la for la in all_labels if la != "O"]))
                    print("\n")
                    print("All 'O':")
                    print(metrics_df(filtered_gold_io, y_all_o, [
                          la for la in all_labels if la != "O"]))
                    print("\n")
                    break
        print("\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Script to evaluate performance of NER. Arguments are all files which should be evaluated.")
    ...
    gold, df_list = make_dataframes(
        args.gold, args.files, annotated_lines_icsd3)
